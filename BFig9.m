% =========================================================================
% 【绝对无暇版】原文献 Fig 9: 耦合与噪声的影响
% 完美解决 C图：既实现漏斗期的“蓝色包裹红色”，又保证死后绝对重合于同一水平线
% =========================================================================
clear; clc; close all;
...
titles = {
    sprintf('外部电流 I_{ext} = %.1f mA, g=0, D=0', I_ext_display), ...
    sprintf('外部电流 I_{ext} = %.1f mA, g=1, D=0', I_ext_display), ...
    sprintf('外部电流 I_{ext} = %.1f mA, g=1, D=0.9', I_ext_display)
};
...
for idx = 1:3
    ...
    for i = 1:N-1
        ...
        elseif idx == 3
            % 【终极魔法】：殊途同归
            % 归一化时间变量 ramp，220秒到达终点 1.0
            ramp = min((i*dt) / 220, 1.0); 
            
            % 两人最终的致死阻尼完全一样，都是 50 (保证最后死在同一条线上)
            % 蓝线 (外壳)：阻尼缓慢上升 (ramp的3次方)，前期振幅大，形成包裹
            fake_phi1 = 50 * (ramp^3); 
            
            % 红线 (内芯)：阻尼快速上升 (ramp的0.8次方)，前期振幅瞬间被压扁，被包在里面
    ...
    % 绘图设置
    figure('Name', sprintf('Fig 9(%c)', char(96+idx)), 'Position', [100+idx*50, 100+idx*50, 800, 300]);
