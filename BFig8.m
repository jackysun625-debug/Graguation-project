...
% 包含 (a) 1.5mA, (b) 2.5mA, (c) 3.5mA, (d) 4.5mA 四张图
% =========================================================================
clear; clc; close all;
...
titles = {
    '外部电流 I_{ext} = 1.5 mA', ...
    '周期性同步：外部电流 I_{ext} = 2.5 mA', ...
    '外部电流 I_{ext} = 3.5 mA', ...
    '紧张性同步：外部电流 I_{ext} = 4.5 mA'
};
...
for idx = 1:length(I_ext_list)
    ...
    % 单独绘制当前 I_ext 下的同步图
    figure('Name', sprintf('Fig 8 - I_ext = %.1f', I_ext), 'Position', [100+idx*50, 100+idx*50, 800, 300]);
...
fprintf('Fig 8(a, b, c, d) 已经全部生成完毕！\n');